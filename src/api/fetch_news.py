...
def make_request(news, category, date, language, country):
    ...
    if news["name"] == "NewsApi":
        return [{"Author": article['author'], "Title": article['title'], "Published At": article['publishedAt'], "Content": article['content'], "Category": category, "ApiSource": news['name'], "URL": article["url"]} for article in articlesList]
    elif news["name"] == "GNews":
       return [{"Author": article['source']['name'], "Title": article['title'], "Published At": article['publishedAt'], "Content": article['content'], "Category": category, "ApiSource": news['name'], "URL": article["url"]} for article in articlesList]
